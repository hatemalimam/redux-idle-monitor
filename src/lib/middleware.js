...
export const createMiddleware = context => {
  ...
  const { nextIdleStatusAction, lastIdleStatusAction } = translateBlueprints({
  ...
  return store => {
    ...
    if (IS_DEV) {
      invariant(
        startActivityDetection,
        "createDetection should return startActivityDetection"
      );
      invariant(
        stopActivityDetection,
        "createDetection should return stopActivityDetection"
      );
    ...
    return next => action => {
      ...
      const scheduleTransition = idleStatus => {
        ...
        invariant(
          delay,
          `must return an idle status delay for idleStatus === '${idleStatus}'`
        );
        invariant(
          typeof delay === "number",
          `idle status delay must be a number type for idleStatus === '${idleStatus}'`
        );
        ...
        let nextCancelMessage = cancelledAt =>
          `${NEXT_IDLE_STATUS} action cancelled before ${delay} MS delay by dispatcher, lastActive: ${new Date().toTimeString()}, cancelledAt: ${cancelledAt}`;
