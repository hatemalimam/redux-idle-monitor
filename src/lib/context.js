...
import {
  ...
  getUseLocalState,
  getUseWebRTCState,
  getUseWebSocketsState,
  getThresholds,
  getLevel
} from "./defaults";
import {
  translateBlueprintsWith,
  translateBlueprintTypesWith
} from "redux-blueprint";
...
const validateContext = (libContext, appContext) => {
  ...
  invariant(
    typeof thresholds.mouse === "number",
    "thresholds.mouse must be a number corresponding to pixels"
  );
  invariant(
    typeof thresholds.phaseOffMS === "number",
    "thresholds.phaseOffMS must be a number corresponding to minimum milliseconds between updates to redux"
  );
  invariant(
    typeof thresholds.phaseOnMS === "number",
    "thresholds.phaseOnMS must be a number corresponding to minimum milliseconds between updates to redux"
  );
};

const configureInitialState = libContext => appContext => {
  return new Map({
    idleStatus: IDLESTATUS_ACTIVE,
    isRunning: false,
...
export default function createContext({
  ...
  const libOpts = {
    libName,
    validateContext,
    configureAppContext: libContext => appOpts => appOpts,
...
const validateLibOpts = libOptsRaw => {
  ...
  invariant(
    ...
    "validateContext must be a function"
  );

  invariant(configureAppContext, "configureAppContext must exist");
  invariant(
    typeof configureAppContext === "function",
    "configureAppContext must be a function"
  );

  invariant(configureInitialState, "configureInitialState must exist");
  invariant(
    typeof configureInitialState === "function",
    "configureInitialState must be a function"
  );
};
...
function configureContext(libOpts) {
  ...
  return appOpts => {
    ...
    const translateBlueprintType = blueprintType =>
      `${cleanActionName(libName)}_${cleanActionName(
        appName
      )}_${cleanActionName(blueprintType)}`;
    const translateBlueprintTypes = translateBlueprintTypesWith(
      translateBlueprintType
    );
    const translateBlueprints = translateBlueprintsWith(translateBlueprintType);

    const libContext = {
      log: createLogger({ libName, level }),
      libName,
      appName,
...
function createLogger({ libName, level }) {
  const _formatMessage = ({ level, message, obj }) => {
    ...
    return _formatLog(
      obj
        ? `${level}: '${message}' => ${JSON.stringify(obj)}`
        : `${level}: '${message}'`
    );
  };
  ...
  return process.env.NODE_ENV !== "production"
    ? {
        trace: (obj, message) =>
          level === "trace"
            ? console.trace(_formatMessage({ level: "trace", message, obj }))
            : noop(),
        debug: (obj, message) =>
          ["trace", "debug"].indexOf(level) !== -1
            ? console.log(_formatMessage({ level: "debug", message, obj }))
            : noop(),
        info: (obj, message) =>
          ["trace", "debug", "info"].indexOf(level) !== -1
            ? console.info(_formatMessage({ level: "info", message, obj }))
            : noop(),
        warn: (obj, message) =>
          ["trace", "debug", "info", "warn"].indexOf(level) !== -1
            ? console.warn(_formatMessage({ level: "warn", message, obj }))
            : noop(),
        error: (obj, message) =>
          ["trace", "debug", "info", "warn", "error"].indexOf(level) !== -1
            ? console.error(_formatMessage({ level: "error", message, obj }))
            : noop()
      }
